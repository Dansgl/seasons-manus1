...
export default function Home() {
  ...
  const { data: products } = useQuery<SanityProduct[]>({
    ...
    <div className="min-h-screen bg-[#FDFBF7]">
      ...
      <nav className="absolute top-0 left-0 right-0 z-50 border-b border-white/20 bg-transparent backdrop-blur-sm">
        <div className="container mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
          <Link href="/">
            <img src="/seasons-logo-bold.png" alt="SEASONS" className="h-7 md:h-8" />
          </Link>
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              ...
              <SheetContent side="right" className="w-[280px] p-0">
                <SheetHeader className="p-4 border-b border-neutral-100">
                  <SheetTitle className="text-left font-light tracking-wide">Seasons</SheetTitle>
                </SheetHeader>
                <div className="py-4">
                  <Link
                    href="/catalog"
                    onClick={() => setMobileMenuOpen(false)}
                    className="block py-3 px-4 text-sm text-neutral-600 hover:text-neutral-900 font-medium"
                  >
                    Browse
                  </Link>
                  <Link
                  ...
                  {user ? (
                    ...
                    <Link
                      href="/login"
                      onClick={() => setMobileMenuOpen(false)}
                      className="block py-3 px-4 text-sm text-neutral-900 font-medium"
                    >
                      Sign In
                    </Link>
                  )}
              ...
              <Link key={product._id} href={`/product/${product.slug}`}>
                <Card className="group overflow-hidden cursor-pointer hover:shadow-xl transition-all duration-300">
                  <div className="aspect-square overflow-hidden bg-neutral-100">
                    <img
                      src={product.mainImage ? urlFor(product.mainImage).width(800).height(800).auto("format").url() : "https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?w=800&h=800&fit=crop"}
                      alt={product.name}
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                  </div>
                  <div className="p-6">
                    <p className="text-xs text-neutral-500 uppercase tracking-wide mb-1">{product.brand?.name}</p>
